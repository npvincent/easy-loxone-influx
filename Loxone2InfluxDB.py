...
def ParseLogData(data, from_zone, to_zone, debug=False):
    ...
    local = datetime.strptime(ParsedData['TimeStamp'].decode(
        'utf-8'), b'%Y-%m-%dT%H:%M:%SZ'.decode('utf-8'))
    ...
    # Value Extraction
    ParsedData['Value'] = rx.findall(
        data[EndAlias+1:EndValue].decode('utf-8'))[0]
...
def main(host, port, ssl, verify, debug=False):
    print('Loxone2Influx Logging Bridge Starting')
    print(F'Connecting to influx database [{dbname}] on [{dbuser}@{host}:{port}]')

    ## """Instantiate a connection to the InfluxDB."""
    client = InfluxDBClient(
        host, port, dbuser, dbuser_code, dbname, ssl, verify)
...
if __name__ == '__main__':
    args = parse_args()
    main(host=args.host, port=args.port, ssl=args.ssl,
